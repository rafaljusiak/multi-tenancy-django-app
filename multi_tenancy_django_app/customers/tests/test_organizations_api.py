...
def test_organizations_detail(rest_client, organization_a, organization_b):
    response = rest_client.get(reverse("api:organizations-detail", args=(organization_a.id,)))
    assert response.status_code == 200
    assert response.json()['id'] == organization_a.id

def test_organizations_detail_cannot_access_another_tenant(rest_client, organization_a, organization_b):
    response = rest_client.get(reverse("api:organizations-detail", args=(organization_b.id,)))
